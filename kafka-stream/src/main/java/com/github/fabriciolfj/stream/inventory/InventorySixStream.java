...
import com.github.fabriciolfj.util.InventoryTimestampExtractor;
...
public class InventorySixStream {
    ...
    public KStream<String, InventoryMessage> kstreamInventory(StreamsBuilder builder) {
        ...
        var inventoryStream = builder.stream("t.commodity.inventory", Consumed.with(stringSerde, inventorySerde, inventoryTimestampExtractor, null));

        inventoryStream
                .mapValues((k,v) -> v.getType().equalsIgnoreCase("ADD") ? v.getQuantity() : -1 * v.getQuantity())
                .groupByKey().windowedBy(TimeWindows.of(windowLength).advanceBy(hopLenght))
                .reduce(Long::sum, Materialized.with(stringSerde, longSerde)).toStream()
                .through("t.commodity.inventory-total-five2", Produced.with(windowSerde, longSerde))
