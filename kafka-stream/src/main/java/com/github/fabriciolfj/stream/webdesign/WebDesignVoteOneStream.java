package com.github.fabriciolfj.stream.webdesign;

import com.github.fabriciolfj.domain.integration.broker.model.WebColorVoteMessage;
import com.github.fabriciolfj.domain.integration.broker.model.WebDesignVoteMessage;
import com.github.fabriciolfj.domain.integration.broker.model.WebLayoutVoteMessage;
import com.github.fabriciolfj.util.WebColorVoteTimestampExtractor;
import com.github.fabriciolfj.util.WebLayoutVoteTimestampExtractor;
...
/*
* A chave foi definida como sendo o username (la no producer)
* */
@Configuration
public class WebDesignVoteOneStream {
    ...
    public KStream<String, WebDesignVoteMessage> kStreamWebDesignVote(StreamsBuilder builder) {
        ...
        joinTable.groupBy((username, votedDesign) -> KeyValue.pair(votedDesign.getColor(), votedDesign.getColor()))
                .count().toStream().print(Printed.<String, Long>toSysOut().withLabel("Vote on - color"));

        joinTable.groupBy((username, votedDesign) -> KeyValue.pair(votedDesign.getLayout(), votedDesign.getLayout()))
                .count().toStream().print(Printed.<String, Long>toSysOut().withLabel("Vote on - layout"));
